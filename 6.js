...
async function loadPage(pageNumber) {
    ...
    // https://www.mashangpa.com/api/problem-detail/6/data/?page=1
    const data = await fetch(`https://www.mashangpa.com/api/problem-detail/6/data/?${queryString}`, {
        headers: window.hhh,
    })
    return data.json()
}
...
(async function () {
    for (let i = 1; i < 21; i++) {
        ...
        const arr = JSON.parse(xxxxoooo(data.t)).current_array
        arr.forEach(e => {
            sum += e
        })
    }
