...
async function loadPage(page) {
    const prefix = md5("xialuo" + time)
    const x = encodeURIComponent(sha256(prefix + "xxoo"))
    console.log("x=", x)
    const res = await fetch(`https://www.mashangpa.com/api/problem-detail/7/data/?page=${page}&x=${x}`, {
        headers: {
            ...headers,
            "ts": time,
            "M": prefix
    ...
    // console.log(`https://www.mashangpa.com/api/problem-detail/7/data/?page=${page}&x=${x}`)
    return res.json()
}
var s = 0;
(async function () {
    for (let i = 1; i < 21; i++) {
        ...
        const r = data.r
        const decrypt = JSON.parse(xxxxoooo(r))
        decrypt.current_array.forEach(e => {
            s += e
        })
    }
    console.log(s)
